...
export default function Home() {
  ...
  const [searchResults, setSearchResults] = useState<{
    ...
    spirits: { id: string; name: string; distillery: string; journalId: string; journalName: string }[];
  }>({ journals: [], spirits: [] });
  ...
  useEffect(() => {
    ...
    async function performGlobalSearch() {
      try {
        // 1. Fetch matching journals
        const allJournals = await db.journals.toArray();
        const matchingJournals = allJournals
        ...
        const matchingSpirits = allSpirits
          ...
          .map(s => {
            const journal = allJournals.find(j => j.id === s.journalId);
            return {
              id: s.id,
              name: s.name,
        ...
        >
          ...
          <div ref={searchRef} className="flex-1 max-w-sm md:max-w-xl lg:max-w-3xl mx-auto relative w-full">
            ...
            {globalSearchQuery.trim() !== '' && (
              <div className="absolute top-10 left-0 right-0 bg-[#224229] border border-[#C59B27]/40 rounded-lg shadow-2xl z-50 p-2 max-h-64 overflow-y-auto divide-y divide-white/5">
                {searchResults.journals.length === 0 && searchResults.spirits.length === 0 ? (
                  ...
                  <>
                    {searchResults.journals.length > 0 && (
                      <div className="py-1">
                        <p className="text-[10px] uppercase font-bold text-[#C59B27] tracking-wider px-2 mb-1">Journals</p>
                        {searchResults.journals.map(j => (
                          <button
                            ...
                            className="w-full text-left text-xs text-[#E8D5B7] hover:bg-white/5 px-2 py-1.5 rounded transition-colors block cursor-pointer"
                          >
                            📖 {j.name}
                          </button>
                        ))}
                    ...
                    {searchResults.spirits.length > 0 && (
                      <div className="py-1">
                        <p className="text-[10px] uppercase font-bold text-[#C59B27] tracking-wider px-2 mb-1">Spirits</p>
                        {searchResults.spirits.map(s => (
                          <button
                            ...
                            className="w-full text-left text-xs text-[#E8D5B7] hover:bg-white/5 px-2 py-1.5 rounded transition-colors block cursor-pointer"
                          >
                            🥃 <span className="font-semibold">{s.name}</span> <span className="text-white/40">({s.distillery} • {s.journalName})</span>
                          </button>
                        ))}
                      </div>
                    )}
